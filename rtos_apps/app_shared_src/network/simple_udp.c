...
void ICACHE_FLASH_ATTR network_before_start() {
	if (netif_default)
    		netif_set_hostname(netif_default, (char*)default_name);
...
void network_data_received(void *arg, char *data, unsigned short len)
{
     ...
     *peer_uid = 0; // replace \n by 0.
     ...
     while (data) {
      char *temp, *cmd_type_str, *action_id, *action_value=0;
	...
	printf("E > No such action: %s\n\r", action_id);
      ...
      *temp = 0; // replace \t by 0.
      action_value = data;
      int value = atoi(action_value);

     //printf("D > action_value: %d\n", value);
